...
public interface ProductAuctionDao extends BaseMapper<ProductAuction> {
    ...
    @Select("select * from product_auction  where auditStatus=1 and buildOrder=1 and endTime < sysdate()")
    List<ProductAuction> selectByOver();

    @Update("update  product_auction set buildOrder=2 where buildOrder=1 and id=#{id}")
    ...
    @Update("update  product_auction set endTime=#{endTime}  where buildOrder=1 and id=#{id}")
    int updateEndTime(@Param("id") Long id,@Param("endTime") String endTime);
}

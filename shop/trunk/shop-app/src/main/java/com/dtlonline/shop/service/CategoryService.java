...
public class CategoryService extends BaseService {
    ...
    public List<CategoryDTO> queryCategoryList(Integer type) {
        ...
        Integer currentType = TypeEnum.PRODUCT_PURCHASE.getValue().equals(type)
                || TypeEnum.PRODUCT_SPOT.getValue().equals(type)
                || TypeEnum.PRODUCT_AUCTION.getValue().equals(type)
                || TypeEnum.PRODUCT_DEMAND.getValue().equals(type)
                || TypeEnum.PRODUCT_SUPPLY.getValue().equals(type)? TypeEnum.PRODUCT_SPOT.getValue() : type;
    ...
    public List<CategoryDTO> queryCategoryListSuggest(Long categoryId) {
        //查询所有类别，转成dto，下方都用dto做处理，不涉及model
        QueryWrapper queryWrapper = new QueryWrapper();
        queryWrapper.eq("type", SUCCESS.intValue());
        queryWrapper.eq("active", SUCCESS);
        queryWrapper.eq("status", SUCCESS);
        queryWrapper.orderByAsc("`index`");
        List<Category> categorys = categoryDao.selectList(queryWrapper);
        List<CategoryDTO> categoryDtos = ObjectUtils.copy(categorys, CategoryDTO.class);
        ...
        List<CategoryDTO> topCategories = categoryDtos.stream().filter(c -> c.getParentId().equals(categoryId)).collect(Collectors.toList());
    ...
    public CategoryDTOI queryCategorysById(Long categoryId) {
        QueryWrapper wrapper = new QueryWrapper();
        ...
        BeanUtils.copyProperties(category,CategoryDTOi);
        return CategoryDTOi;
    }
    ...
    public void saveCategory(CategoryPackDTO categoryPackDTO, Long userId) {
        ...
        if (CollectionUtils.isNotEmpty(categoryPackDTO.getImgList())) {
            List<Long> imgs = imageRemoteService.batchSaveImage(categoryPackDTO.getImgList()).getData();
    ...
    public List<CategoryAttentionDTO> queryCategoryAttention(Long userId){
        ...
        wrapper.eq("status",SUCCESS);
        wrapper.eq("active",SUCCESS);
        wrapper.eq("type",TypeEnum.PRODUCT_SPOT.getValue());
        wrapper.orderByAsc("createTime");
        List<Category> categoryList = categoryDao.selectList(wrapper);
        Map<Long, AttentionDTO> longAttentionDTOMap = attentionService.queryAttentionMap(userId);
        List<CategoryAttentionDTO> categoryDTOList = new ArrayList();
        categoryList.stream().forEach(c ->{
            CategoryAttentionDTO categoryAttentionDTO = Category.ofToAttention(c);
            boolean isFollow = longAttentionDTOMap.get(c.getId()) != null;
            categoryAttentionDTO.setIsFollow(isFollow ? 1 : 0);
            categoryDTOList.add(categoryAttentionDTO);
        });
        return categoryDTOList;
    }

    public Set<Long> queryChildCategory(Set<Long> categorySet){
        List<Category> categoryList = categoryDao.selectList(Wrappers.<Category>lambdaQuery().in(Category::getParentId, categorySet));
        Set<Long> longSet = queryParentCategory(categoryList.stream().map(Category::getId).collect(Collectors.toSet()));
        return longSet;
    }

    public Set<Long> queryParentCategory(Set<Long> categorySet){
        if (categorySet.isEmpty()){
            return Collections.EMPTY_SET;
        }
        Set<Long> childSet = new HashSet<>();
        Set<Long> parentSet = queryChildCategory(categorySet);
        childSet.addAll(categorySet);
        childSet.addAll(parentSet);
        return childSet;
    }

    public Category queryParentCategory(Long substId){
        Category category = categoryDao.selectOne(Wrappers.<Category>lambdaQuery().eq(Category::getId, substId));
        Category categoryList = queryAllCategoriesByParentId(category.getParentId());
        return categoryList;
    }

    private Category queryAllCategoriesByParentId(Long parentId) {
        if (parentId==null || parentId ==0) {
            return null;
        }
        Category currentCategory = categoryDao.selectOne(Wrappers.<Category>lambdaQuery().eq(Category::getId, parentId));
        Category parentCategory = queryAllCategoriesByParentId(currentCategory.getParentId());
        return parentCategory == null ? currentCategory : parentCategory;
    }
}

...
public class ProductService extends BaseService {
    ...
    public List<ProductListDTO> queryProductIndexList() {
        ...
        queryWrapper.eq("type", TypeEnum.PRODUCT_SPOT.getValue());
    ...
    public ProductDetailDTO queryProductDetail(Long productId, Long userId) {
        ...
        if (product == null) {
            Optional.ofNullable(product).orElseThrow(() ->
                    new ProductException(ViewCode.PRODUCT_FAILURE.getCode(),
                            ViewCode.PRODUCT_FAILURE.getMessage()));
            return null;
        }
    ...
    public Map<Long, ProductInfoDTO> queryProductInfoByIds(Set<Long> ids) {
        ...
        Set<Long> urlIds = list.stream().map(Product::getImgs)
        ...
        products.values().forEach(p -> {
            Optional.ofNullable(p.getType()).filter(type -> type.equals(TypeEnum.PRODUCT_AUCTION.getValue())).ifPresent(type -> {
    ...
    public Map<Long, ProductInfoDTO> queryProductGroupbuyByIds(Set<Long> ids) {
        QueryWrapper wrapper = new QueryWrapper();
        wrapper.in("id", ids);
        List<Product> list = productDao.selectList(wrapper);
        Map<Long, ProductInfoDTO> products = list.stream()
                .collect(Collectors.toMap(Product::getId, Product::of));
        ...
        if (CollectionUtils.isNotEmpty(urlIds)){
            Map<Long, String> urls = imageRemoteService.queryUrls(urlIds);
            products.values().forEach(p -> {
                Set<Long> productUrlIds = Arrays.stream(p.getImgs().split(",")).map(Long::valueOf).collect(Collectors.toSet());
                List<String> imageList = new ArrayList<>(productUrlIds.size());
                productUrlIds.forEach(id -> imageList.add(urls.get(id)));
                p.setImgs(imageList.isEmpty() ? "" : imageList.get(0));
                p.setCategoryName(categoryMap.get(p.getCategoryId()).getTitle());
            });
        }
        return products;
    }
    ...
    public Integer updateProductForId(Long productId, ProductRecordDTO productRecordDTO, Long userId) {
        ...
        Optional.ofNullable(result).filter(r -> !r.equals(0))
                .orElseThrow(() -> new ProductException(ViewCode.PRODUCT_FAILURE.getCode(), "修改商品信息失败,请联系管理员"));
        return result;
    }

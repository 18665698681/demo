...
public class StoreGoodsRequireService extends ServiceImpl<StoreGoodsRequireMapper, StoreGoodsRequire> {
    ...
    public void publishOne(StoreGoodsRequireDTOI publishDTO, Long userId) {
        ...
        //平台仓单需要把仓单拆出一部分，进行冻结
        if (StoreGoodsRequireTypeEnum.PLATFORM.getValue().equals(publishDTO.getType())) {
            Long newStoreGoodsId = storeGoodsSplitService.split(publishDTO.getStoreGoodsId(), StoreGoodsBizTypeEnum.PROVIDING.getValue(), publishDTO.getQuantity(), userId, SequenceUtils.getSequence());
    ...
    private void handleRequireExtra(StoreGoodsRequireDTO record) {
        ...
        //显示的数量替换为可用数量
        record.setQuantity(record.getValidQuantity());

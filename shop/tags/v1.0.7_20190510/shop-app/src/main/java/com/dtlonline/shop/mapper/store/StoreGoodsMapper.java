...
public interface StoreGoodsMapper extends BaseMapper<StoreGoods> {
    ...
    @Update("UPDATE store_goods SET validQuantity = validQuantity+#{calculateQuantity} WHERE id = #{storeGoodsId} AND validQuantity = #{validQuantity}")
    Integer calculateValidQuantity(@Param("storeGoodsId") Long storeGoodsId,@Param("calculateQuantity") Integer calculateQuantity,@Param("validQuantity") Integer validQuantity);

...
public class AuctionCenterService extends BaseService implements AuctionApplyStatus {
    ...
    private ShippingAddressDTO checkUserAddress(Long addressId) {
        ShippingAddressDTO address = userRemoteService.queryUserShippingAddress(addressId);
        if (address == null) {
    ...
    private void checkUserBuyer(ProductAuction productAuction, AuctionUserBuyer buyer, Long userId) {
        ...
        if(AuctionType.DOWN.getCode().equals(productAuction.getAuctionType())){
            downBuyerPriceCheck(productAuction, buyer, userId,product);
        }
    ...
    private void downBuyerPriceCheck(ProductAuction productAuction, AuctionUserBuyer buyer, Long userId, Product product) {
        if (buyer.getPrice().compareTo(product.getUnitPrice()) == 1) {
            throw new AuctionException(ViewCode.FAILURE.getCode(), "不能高于起拍价");
        }

        BigDecimal subPrice = product.getUnitPrice().subtract(buyer.getPrice());
        if (subPrice.compareTo(BigDecimal.ZERO) == 1 && subPrice.divideAndRemainder(productAuction.getMinAddPrice())[1].compareTo(BigDecimal.ZERO) != 0) {
            throw new AuctionException(ViewCode.FAILURE.getCode(), "减价必须为 " + productAuction.getMinAddPrice() + " 的整数倍");
        }
    }

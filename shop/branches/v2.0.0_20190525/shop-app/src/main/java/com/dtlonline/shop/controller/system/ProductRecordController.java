...
public class ProductRecordController extends BaseController {
    ...
    public RestResult<ProductDTO> queryProductRecordList(@RequestParam(value = "current",defaultValue = "1") Integer current,@RequestParam(value = "size",defaultValue = "10") Integer size,String title,Integer status) {
        return RestResult.success(productRecordService.queryProductRecordList(current,size,title,status));
    }

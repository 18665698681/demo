    ...
    public ProductDetailDTO queryProductDetail(Long productId, Long userId) {
        ...
        if (product != null) {
            ...
            productDetailDTO.setProductStandardList(productStandardRecords)
                    ...
                    .setPersonalCertificate(authRemoteService.queryRealnameAuthStatus(userId));
            ...
            Optional.ofNullable(product.getType()).filter(type -> type.equals(99)).ifPresent(t -> {
                UserAuthDetailDTO userEntity = infosRemoteService.queryUserAuthDetail(product.getUserId());
                userEntity.setEnterpriseCertification(authRemoteService.queryEnterpriseAuthStatus(product.getUserId()));
                userEntity.setPersonalCertificate(authRemoteService.queryRealnameAuthStatus(product.getUserId()));
                userEntity.setFieldCertification(authRemoteService.querySiteAuthStatus(product.getUserId()));
